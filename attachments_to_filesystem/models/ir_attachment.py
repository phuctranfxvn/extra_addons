...
class IrAttachment(models.Model):
    ...
    def _attachments_to_filesystem_init(self):
        ...
        if location:
            ...
            ir_model_data._update('ir.config_parameter', module_name, {
        ...
        if self.env['ir.config_parameter'].get_param(
            ...
            self._attachments_to_filesystem_cron()
            return
        ...
        move_cron = self.env.ref(
            module_name + '.cron_move_attachments')
        move_cron.write(
            {
                'nextcall':
    ...
    def _attachments_to_filesystem_cron(self, limit=10000):
        ...
        for counter, attachment_id in enumerate(attachment_ids, start=1):
            attachment_data = ir_attachment.read(
                [attachment_id], ['datas', 'res_model'])[0]
            if attachment_data['res_model'] and not self.env.get(
